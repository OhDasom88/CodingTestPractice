...
def solution(n:int, money:list):
    ...
    def sol(m:int, coins:list):
        ...
        for i in range(m//coin+1):
            new_m = m-i*coin
            if new_m ==0 : answer +=1; continue
            key = (new_m, tuple(coins))
            if  key not in cache:
                cache.update({key:sol(new_m, coins.copy())}) 
            answer += cache.get(key)
        return answer
    return sol(n,sorted(money))%1000000007
ans = solution(5, [1,2,5])#4
print(ans)